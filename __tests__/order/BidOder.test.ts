...
describe('BidOrder', () => {
  it('buildEip712Data', async () => {
    ...
    const bidOrder = new BidOrder(nftMarketplaceSdk, 'itemId', 'itemAmount', 'cryptoCurrencyId', 'cryptoCurrencyAmount', 'userWallet', 'startTimeUtc', 'endTimeUtc');

    // mocking unrelated method
    const bidOrderFetchRequiredData = jest
      .spyOn(bidOrder, 'fetchRequiredData').mockImplementation(async () => {
        bidOrder.itemData = {
          type: 'ERC721',
  ...
  it('arrayify', async () => {
    ...
    expect(result).toEqual(["userWallet", "0xCryptoCurrencyAddress", "0xItemContractAddress", "startTimeUtc", "endTimeUtc"]);
    expect(bidOrderFetchRequiredData).toBeCalledTimes(1);
  });
});
